    ...
    if (lex) {
        ...
        if (saved_text && saved_text[0]) {
            if (lex->saved_text.length <= 20) {
                snprintf(msg_with_context, JSON_ERROR_TEXT_LENGTH, "%s %s %s",
                         msg_text, "near",saved_text);
            ...
            if (lex->stream.state == STREAM_STATE_ERROR) {
                ...
                snprintf(msg_with_context, JSON_ERROR_TEXT_LENGTH, "%s %s",
                         msg_text,"near end of file");
                msg_with_context[JSON_ERROR_TEXT_LENGTH - 1] = '\0';
                result = msg_with_context;
            }
        }
...
static int lex_scan_number(lex_t *lex, int c, json_error_t *error) {
    ...
    if (!(lex->flags & JSON_DECODE_INT_AS_REAL) && c != '.' && c != 'E' && c != 'e') {
        ...
        if(end == saved_text + lex->saved_text.length)
        {
        }
        else{os_printf("end assert fail");while(1);}
