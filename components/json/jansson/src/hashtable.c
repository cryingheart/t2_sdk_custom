...
#if 1
static JSON_INLINE void hash_list_init(list_t *list) {
    list->next = list;
    list->prev = list;
}
#endif
...
int hashtable_init(hashtable_t *hashtable) {
    ...
    hash_list_init(&hashtable->list);
    hash_list_init(&hashtable->ordered_list);
...
int hashtable_set(hashtable_t *hashtable, const char *key, json_t *value) {
    ...
    if (pair) {
        ...
        pair->value = value;
        hash_list_init(&pair->list);
        hash_list_init(&pair->ordered_list);
