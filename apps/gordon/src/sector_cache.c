               ...
               const unsigned char *sector, size_t len)
{
    ...
    if(program != 0) {
        pr_info("Programming sector %u:", sector_address / SECTORSIZE);
        for(unsigned page_index = 0; page_index < SECTORSIZE / PAGESIZE; page_index++) {
            if((program & BIT(page_index)) != 0) {
                if(OS_DEBUG >= PR_LEVEL_INFO)
                    pr_log_simple(" %2u", page_index);
...
static struct sector_cache * __attribute__((__returns_nonnull__))
...
void
sector_cache_erase_sector(struct spi_mem_device *dev, uint32_t address)
{
    for(size_t idx = 0; idx < sector_cache_size; idx++) {
        struct sector_cache *c = &sector_cache[idx];
        if (c->address == address) {
            if(c->state == VALID || c->state == DIRTY) {
                c->state = EMPTY;
            }
            c->address = 0;
        }
    }
    spi_sector_erase(dev,address);
}
